...
class policy_estimator_network():
    ...
    def reinforce(self, env, num_episodes=2000,
        ...
        while ep < num_episodes:
            ...
            pool_mapping = [(env) for i in range(batch_size)]

            batch = starmap(self.run_episode, pool_mapping)

            while not self.batch_queue.empty():
                (states, actions, rewards) = self.batch_queue.get()
    ...
    @jit(target ="cuda")                          
    def reinforce_multiprocess(self, envs, num_episodes=2000, gamma=0.999):
        # Set up lists to hold results
        total_rewards = []
        batch_rewards = []
        batch_actions = []
        ...
        while ep < num_episodes:
            ...
            processes = [ ]
            for env in envs:
                t = multiprocessing.Process(target= self.run_episode, args=(env,))
                t.start()
                t.join()
